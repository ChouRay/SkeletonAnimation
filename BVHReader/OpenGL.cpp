...
OpenGL::OpenGL()
	: m_frame(0),m_nextframe(-1),isRead(false)
{
	this->m_bvhopengl = new BvhOpenGL();
}
OpenGL::~OpenGL()
{	CleanUp();
}
...
{	
	...
	m_bvhopengl->DisplayScene();
	m_bvhopengl->DrawGround();
	if(isRead ==true)
	{
		m_nextframe = (m_nextframe+1)%m_frame;
		m_bvhopengl->DrawSkeleton(m_nextframe);
	}
	glFlush();									 // 更新窗口
	SwapBuffers(hDC);								 // 切换缓冲区
}

void OpenGL::IsReadFile()
{
	if(m_bvhopengl->GetFileName("Jumping - Spinning Kick"))
	{
		this->m_frame = m_bvhopengl->GetFrameNum();
		isRead = true;
	}
}
